<aside id="logo-sidebar" class="fixed top-0 left-0 z-40 w-64 h-screen pt-20 transition-transform -translate-x-full glass-panel border-r sm:translate-x-0" aria-label="Sidebar">
   <div class="h-full px-3 pb-4 overflow-y-auto bg-transparent">
      <ul class="space-y-2 font-medium">
         ...
         <li>
            <a href="{{ route('partner.dashboard') }}" class="relative flex items-center p-2 rounded-xl transition-all duration-200 {{ request()->routeIs('partner.dashboard') ? 'bg-primary-50 text-primary-700 shadow-sm' : 'text-gray-900 dark:text-white hover:bg-gray-100/50 hover:pl-3 group' }}">
               @if(request()->routeIs('partner.dashboard'))
                  <div class="absolute left-0 w-1 h-6 bg-primary-600 rounded-r-full"></div>
               @endif
               <svg class="w-5 h-5 {{ request()->routeIs('partner.dashboard') ? 'text-primary-600' : 'text-gray-500' }} transition duration-75 group-hover:text-gray-900 dark:group-hover:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 21">
         ...
         @elseif(Auth::user()->role == 'PEMILIK' || Auth::user()->role == 'PETERNAK')
         <li>
            <a href="{{ route('dashboard') }}" class="relative flex items-center p-2 rounded-xl transition-all duration-200 {{ request()->routeIs('dashboard') ? 'bg-primary-50 text-primary-700 shadow-sm' : 'text-gray-900 dark:text-white hover:bg-gray-100/50 hover:pl-3 group' }}">
               @if(request()->routeIs('dashboard'))
                  <div class="absolute left-0 w-1 h-6 bg-primary-600 rounded-r-full"></div>
               @endif
               <svg class="w-5 h-5 {{ request()->routeIs('dashboard') ? 'text-primary-600' : 'text-gray-500' }} transition duration-75 group-hover:text-gray-900 dark:group-hover:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 21">
                  <path d="M16.975 11H10V4.025a1 1 0 0 0-1.066-.998 8.5 8.5 0 1 0 9.039 9.039.999.999 0 0 0-1-1.066h.002Z"/>
                  <path d="M12.5 0c-.157 0-.311.01-.565.027A1 1 0 0 0 11 1.02V10h8.975a1 1 0 0 0 1-.935c.013-.188.028-.374.028-.565A8.51 8.51 0 0 0 12.5 0Z"/>
               </svg>
         ...
         <li>
            <a href="{{ route('animals.index') }}" class="relative flex items-center p-2 rounded-xl transition-all duration-200 {{ request()->routeIs('animals.*') ? 'bg-primary-50 text-primary-700 shadow-sm' : 'text-gray-900 dark:text-white hover:bg-gray-100/50 hover:pl-3 group' }}">
               @if(request()->routeIs('animals.*'))
                  <div class="absolute left-0 w-1 h-6 bg-primary-600 rounded-r-full"></div>
               @endif
               <svg class="flex-shrink-0 w-5 h-5 {{ request()->routeIs('animals.*') ? 'text-primary-600' : 'text-gray-500' }} transition duration-75 group-hover:text-gray-900 dark:group-hover:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 18">
        ...
        @if(in_array(Auth::user()->role, ['PEMILIK', 'PETERNAK']))
        <li x-data="{ open: {{ (request()->routeIs('inventory.*') || request()->routeIs('invoices.*') || request()->routeIs('hpp-manual-costs.*')) ? 'true' : 'false' }} }">
         ...
         <li x-data="{ open: {{ (request()->routeIs('users.*') || request()->routeIs('partners.*') || request()->routeIs('masters.*') || request()->routeIs('settings.dashboard') || request()->routeIs('admin.site-content.*')) ? 'true' : 'false' }} }">
             <button @click="open = !open" type="button" class="flex items-center w-full p-2 text-base text-gray-900 transition duration-75 rounded-lg group hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700">
                <svg class="flex-shrink-0 w-5 h-5 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
         ...
         <!-- Operator Routes -->
         @if(Auth::user()->role == 'STAF' || Auth::user()->role == 'PETERNAK' || Auth::user()->role == 'PEMILIK')
         <li>
            <a href="{{ route('scan.index') }}" class="relative flex items-center p-2 rounded-xl transition-all duration-200 {{ request()->routeIs('scan.*') ? 'bg-primary-50 text-primary-700 shadow-sm' : 'text-gray-900 dark:text-white hover:bg-gray-100/50 hover:pl-3 group' }}">
               @if(request()->routeIs('scan.*'))
                  <div class="absolute left-0 w-1 h-6 bg-primary-600 rounded-r-full"></div>
               @endif
               <svg class="w-5 h-5 {{ request()->routeIs('scan.*') ? 'text-primary-600' : 'text-gray-500' }} transition duration-75 group-hover:text-gray-900 dark:group-hover:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">
